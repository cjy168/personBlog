...
public class AddNewArticle extends HttpServlet {
	 ...
	 * 获取文件对象，并获取文件的图片
	 */
	protected Article getFileObject(HttpServletRequest request, HttpServletResponse response)
			throws ServletException, IOException {
		/*
		 * 先判断表单中是否设置了enctype属性， enctype属性规定在发送到服务器之前应该如何对表单数据进行编码
		...
		/*
		 * 使用 ServletFileUpload解析器解析上传的数据， 解析的结果是返回一个list<FileItem>集合
		...
		try {
			...
			if (items != null) {
				for (FileItem fileItem : items) {
					/*
					 * fileItem.isFormField()判断当前是否为普通的表单字段 如果是普通的表单字段，返回true 是文件字段，返回false
					 */
					if (!fileItem.isFormField()) {
						...
						if (fileName != null && !fileName.equals("")) {
							/*
							 * 包中的FilenameUtils是工具类 FilenameUtils.getName(str) 表示从输入的路径中找到文件名（包括其后缀）
							 */
							fileName = FilenameUtils.getName(fileName);

							/*
							 * 把文件路径确定在服务器中 getServletContext().getRealPath("/upload") 获得当前运行文件在服务器上的绝对路径
							 */
							...
							/*
							 * fileDirectory:文件的目录 File.separator ：表示文件的分隔符“/” fileName ：文件名
							 */
				...
				fileObj.setBody(items.get(3).getString("utf-8"));
